...
class ChangePasswordEmailCode extends StatefulWidget {
  final String? email;

  const ChangePasswordEmailCode({super.key, required this.email});

  @override
  State<ChangePasswordEmailCode> createState() =>
      _ChangePasswordEmailCodeState();
}

class _ChangePasswordEmailCodeState extends State<ChangePasswordEmailCode> {
      ...
      try {
        ...
        if (code.length == 6) {
          final response = await http
              .post(
                Uri.parse(
                  '${AppConfig.baseUrl}/api/auth/change-password/verify-email-code',
                ),
                headers: {'Content-Type': 'application/json'},
