...
class RSOCreationVerification extends StatefulWidget {
  final String? email;

  const RSOCreationVerification({super.key, required this.email});

  @override
  State<RSOCreationVerification> createState() =>
      _RSOCreationVerificationState();
}

class _RSOCreationVerificationState extends State<RSOCreationVerification> {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (code.length == 6) {
          final response = await http
              .post(
                Uri.parse('${AppConfig.baseUrl}/api/auth/verify/rso-code'),
                headers: {'Content-Type': 'application/json'},
